class ZSAPLINK_DEVC definition
  ...
  create public .
*"* public components of class ZSAPLINK_DEVC
...
*"* do not include other source files here!!!
  ...
  methods DELETEOBJECT
    redefinition .
private section.
*"* private components of class ZSAPLINK_DEVC
*"* do not include other source files here!!!
...
*\---------------------------------------------------------------------/
  ...
  DATA datarow_node TYPE REF TO if_ixml_element. " blackcoffeerider 2011-11-06
...
**>>> blackcoffeerider 2011-11-06
  DATA: lt_packint          TYPE tpak_package_interface_list,
        lr_packint_line     TYPE tpak_package_interface_ref,
        ls_packint_dataline TYPE scompidtln.


  obj->get_interfaces(
    IMPORTING
      e_package_interfaces = lt_packint
*  EXCEPTIONS
*    object_invalid       = 1
*    unexpected_error     = 2
*    intern_err           = 3
...
*            WITH SY-MSGV1 SY-MSGV2 SY-MSGV3 SY-MSGV4.
  ...
  DATA: lt_packperm          TYPE tpak_permission_to_use_list,
        lr_packperm_line     TYPE tpak_permission_to_use_ref_typ,
        ls_packperm_dataline TYPE	scomppdtln.

  obj->get_permissions_to_use(
    IMPORTING
      e_permissions    =  lt_packperm
*  EXCEPTIONS
*    object_invalid   = 1
*    unexpected_error = 2
*    others           = 3
...
**>>> blackcoffeerider 2011-11-06
  LOOP AT lt_packint INTO lr_packint_line.
    lr_packint_line->get_all_attributes( IMPORTING e_package_interface_data = ls_packint_dataline ).
    datarow_node = xmldoc->create_element( 'Interface' ).
    me->setattributesfromstructure( node = datarow_node structure = ls_packint_dataline ).
    rc = root_node->append_child( datarow_node ).
  ENDLOOP.

  LOOP AT lt_packperm INTO lr_packperm_line.
    lr_packperm_line->get_all_attributes( IMPORTING e_permission_data = ls_packperm_dataline ).
    datarow_node = xmldoc->create_element( 'Permission' ).
    me->setattributesfromstructure( node = datarow_node structure = ls_packperm_dataline ).
    rc = root_node->append_child( datarow_node ).
  ENDLOOP.
**<<< blackcoffeerider 2011-11-06
